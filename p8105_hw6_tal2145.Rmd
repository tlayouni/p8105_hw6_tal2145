...
**Using `crossv_mc` to perform the training/testing split and storing the data as a data frame**
...
Based on the above plot showing the variance in prediction error distribution across the three models, it appears the hyptohesized model that I created is optimal, but only slightly better than the model that contains head circumference, length, sex and all interaction terms between them. We can see this because `fit` or my hypothesized model has the lowest RMSE distribution of all three models and this means it is a better predictor of birthweight in thid sample. 
...
weather_bootstrapped %>% 
  summarise(
    ...
    sd_r2 = sd(r.squared),
    error = qnorm(0.975)*sd_r2/sqrt(5000),
    CI_lower = mean_r2 - error,
    CI_upper = mean_r2 + error
  )
```
...
weather_bootstrapped %>% 
  summarise(
    mean_betas = mean(product_logb),
    sd_betas = sd(product_logb),
    error_betas = qnorm(0.975)*sd_betas/sqrt(5000),
    CI_lower = mean_betas - error_betas,
    CI_upper = mean_betas + error_betas
  )
```
